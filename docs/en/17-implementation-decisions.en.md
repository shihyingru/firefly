...
| D-014 | 2026-09-03 | Option D: onboarding push, then self-serve. Day 0 replies with today's queue (zero storage); after the user explicitly taps "yes", the encrypted LINE userId is kept in Redis only, TTL = onboarding_push_days (initially 3) × 24h + 1h; daily queue push, the last one carries self-serve instructions and deletes the entry; push failure deletes it; daily push cap; afterwards the rich-menu postback queue:today serves the queue. Behind line.onboarding_push_enabled, off by default until the doc-10 30-day notice period ends. | Doc 7.1 conflicts with doc 08; this option has the smallest footprint, a fixed expiry and explicit consent | 02, 07, 08, 10 |
